...
const CameraPositionX =0;//Sets the camera perspective for the user
const CameraPositionY = 5; //Sets the camera perspective for the user
const CameraPositionZ = -20;//Sets the camera perspective for the user
var GRAPHICS = new Object();// Holds ALL world graphics
var TEXTURE_FILES_INDEX = new Object();//server assigned texture file INDEX locations
var TEXTURE_FILES = new Array();//server assigned texture files
const RENDERING_BUFFER_TOTAL_FRAMES = 10;//too large a buffer makes for laggy game play
var RENDERING_BUFFER = new Graphics_Buffer_Handler(RENDERING_BUFFER_TOTAL_FRAMES );
const TEXTURE_LOADER = new THREE.TextureLoader();


//GLOBAL Physics variables
var physicsWorld;//the world in sync with server
var rigidBodies = new Object();// Holds synchronized physics objects
var physicsWorld_local;//NOT in sync with server.  Used for visual effects mostly
var rigidBodies_local = new Object();// Holds NON-synchronized physics objects
...
//Input Controller
var GAMEPAD = new ABUDLR({left:{GUIsize:60,callback:GAMEPAD_left_callback},right:{GUIsize:60,callback:GAMEPAD_right_callback,upRight:{color:'black'},downRight:{color:'black'},upLeft:{color:'black'},downLeft:{color:'black'},}});
...
var socket = io();
		...
		socket.on('connect',function(msg){
			/*
			LAUNCH LOADING SCREEN and/or INSTRUCTIONS
			*/
		...
		socket.on('newPlayer',function(msg){
		   ...
		   //TODO: STORE NewID somewhere because it represents a players socketID
			...
			//	OTHER_PLAYERS[NewID] = createBox(msg[NewID],true)}
		   });
...
function serverTextureLoader(filenameArray) {
	...
	for (f=0;f<filenameArray.length;f++) {
 		 texture = TEXTURE_LOADER.load(filenameArray[f]);
...
Graphics_Buffer_Handler = function(TotalFrames){
	...
	this.INDEX_Rw = 7;
	this.INDEX_total = 8;
	this.buffer = new Array(this.TotalFrames);
	this.ready = false;//toggle when no value is null
...
Graphics_Buffer_Handler.prototype.buffering(PhysicsWorld,PhysicsBodies,GraphicsObjects,clock){
	...
	while(this.bufferFrame <= this.TotalFrames ){
		  
		// Step world
		PhysicsWorld.stepSimulation( CLOCK.getDelta(),10);
		
		this.updateBuffer(PhysicsBodies,GraphicsObjects);
	}
	this.ready = true;
...
Graphics_Buffer_Handler.prototype.updateBuffer(PhysicsBodies,GraphicsObjects){
	...
	var IDs = Object.keys(PhysicsBodies);	
	
	this.buffer[this.bufferFrame] = new Array();
	...
	for ( var i = 0; i < IDs.length; i++ ) {
		...
		//get the objects ID
		var id = IDs[i]
		var objGraphic = GraphicsObjects[id];
		var objPhys = PhysicsBodies[id];
		...
		var ms = objPhys.getMotionState();
			...
			if ( ms ) {
				...
				var q = this.transformAux1.getRotation();
					...
					this.buffer[this.bufferFrame][TotalObjs][this.INDEX_Rx] = q.x();
					this.buffer[this.bufferFrame][TotalObjs][this.INDEX_Ry] = q.y();
					this.buffer[this.bufferFrame][TotalObjs][this.INDEX_Rz] = q.z();
					this.buffer[this.bufferFrame][TotalObjs][this.INDEX_Rw] = q.w();
...
function animate() {
	...
	****************************************/
	...
	/*CHASE CAMERA EFFECT*/
		var relativeCameraOffset = new THREE.Vector3(CameraPositionX,CameraPositionY,CameraPositionZ);//camera chase distance
...
function init() {
		...
		initPhysics(physicsWorld_local);

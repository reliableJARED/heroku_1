...
physicsWorldManager.prototype.applyServerUpdates = function (binaryData) {
			...
			/*console.log('server time stamp:',ts);
			console.log('end of last update',this.oldTime);
			console.log('oldTime - server:',this.oldTime - ts )
			console.log('current time',Date.now())	
			console.log('current - server:',Date.now() - ts )*/
			...
			for (var obj = 8;obj<allData;obj +=bytesPerObj) {
				...
				//TESTING
				if (obj < 9) {
				//	console.log('CURRENT:',PWM.rigidBodiesMasterObject[objectData[structObj.id]].BinaryExport_physics());				
				}
				...
				//TESTING
				if (obj < 9) {
					//console.log('UPDATED:',PWM.rigidBodiesMasterObject[objectData[structObj.id]].BinaryExport_physics());	
				//	console.log('*******************************')	
				}
			}
